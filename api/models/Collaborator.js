      ...
      */
      ...
      topCollaborators: function(){
        var query = "SELECT coll.name, Sum(direct_downloads) as total From Collaborators coll, PackageVersions pack, DownloadStatistics downloads Where pack.package_name=downloads.package_name "+
          "and pack.release_date = (SELECT Max(release_date) From PackageVersions where package_name=pack.package_name) and coll.id=pack.maintainer_id and downloads.date >= current_date() - interval '1' month group by maintainer_id order by total desc limit 0,10";
